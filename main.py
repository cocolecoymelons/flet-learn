...
class Dompet(): 
    ...
    def adding(self, nominal, keterangan):
        self.liquid_assets = round(self.liquid_assets + nominal, 2)
        self.add_record(keterangan, nominal, "Tambah") 
        self.save_data()
    ...
    def substract(self, nominal, keterangan):
        if nominal < self.liquid_assets:
            self.liquid_assets = round(self.liquid_assets - nominal, 2)
            self.add_record(keterangan, nominal, "Tarik")
            self.save_data()
    ...
    def saving(self, nominal, keterangan):
        if nominal < self.liquid_assets:
            self.savings = round(self.savings + nominal, 2)
            self.liquid_assets = round(self.liquid_assets - nominal, 2)
            self.add_record(keterangan, nominal, "Tabung")
            self.save_data()
    ...
    def saving_wd(self, nominal, keterangan):
        if nominal <= self.savings:
            self.savings = round(self.savings - nominal, 2)
            self.liquid_assets = round(self.liquid_assets + nominal, 2)
            self.add_record(keterangan, nominal, "WD Tabung")
            self.save_data()
            ...
            print("Saldomu tidak cukup!")
            return False, "Saldomu tidak mencukupi!"
    def add_record(self, keterangan, nominal, tipe_jenis):
        jkt = pytz.timezone('Asia/Jakarta')
        waktu_sekarang = datetime.now(jkt).strftime("%Y:%m:%d || %H:%M")
...
def main(page: ft.Page):
    ...
    def main_page(username):
        ...
        list_tampilan = ft.Column(scroll=ft.ScrollMode.ALWAYS, height=200)
        input_keterangan = ft.TextField(label="Jenis")
        input_nominal = ft.TextField(label="Nominal (dalam $)")
        def refresh_list():
            list_tampilan.controls.clear()
            for item in reversed(dompetku.history):
                tipe_data = item.get("Tipe") 
                warna = "red" if tipe_data in ["Tarik", "WD Tabung"] else "green"
                simbol = "-" if tipe_data in ["Tarik", "WD Tabung"] else "+"
                jam = item.get("waktu", "--:--")
                ket = item.get("Keterangan", "Tanpa keterangan")
                nom = item.get("Nominal", 0)
                list_tampilan.controls.append(
                    ft.Text(f"[{jam}] {ket} : {simbol}${nom}", color=warna)
                )
            page.update()
        def aksi_transaksi(e, tipe):
            ...
            try:
                ...
                keterangan = input_keterangan.value if input_keterangan.value else "Tanpa keterangan"
                if tipe == "Tambah":
                    sukses, pesan = dompetku.adding(nominal, keterangan)
                elif tipe == "Tarik":
                    sukses, pesan = dompetku.substract(nominal, keterangan)
                elif tipe == "Tabung":
                    sukses, pesan = dompetku.saving(nominal, keterangan)
                elif tipe == "WD Tabung":
                    sukses, pesan = dompetku.saving_wd(nominal, keterangan)
                if sukses:
                    dompetku.save_data()
        ...
        page.add(
            ft.Column(
                [
                    ...
                    ft.Divider(),
                    ft.Text("Riwayat Transaksi:", size=18, weight="bold"),
                    list_tampilan,
                    ft.Divider(),
